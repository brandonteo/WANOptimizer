...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true")
    parser.add_argument(
        "--data-reduction-suffixed-files2",
        dest="data_reduction_suffixed_files2",
        action="store_true")
    parser.add_argument(
        "--cache-is-not-flow-specific",
        dest="cache_is_not_flow_specific",
        action="store_true")
    parser.add_argument(
        "--test-suffix-with-added-stuff",
        dest="test_suffix_with_added_stuff",
        action="store_true")
    parser.add_argument(
        "--hash-partial-blocks",
        dest="hash_partial_blocks",
        action="store_true")
    ...
    if args.cache_is_not_flow_specific or args.run_all:
        ...
        passed_tests += run_test(
            ...
            testing_part_1)
        total_tests += 1
    if args.test_suffix_with_added_stuff or args.run_all:
        test_module = test_suffix_with_added_stuff
        passed_tests += run_test(
            test_module.test_suffix_with_added_stuff,
            middlebox_module,
            testing_part_1)
        total_tests += 1
    if args.hash_partial_blocks or args.run_all:
        test_module = hash_partial_blocks
        passed_tests += run_test(
            test_module.hash_partial_blocks,
            middlebox_module,
            testing_part_1)
